...
def api_heartbeat(x_user_token: Optional[str] = Header(None)):
    """Pinged every ~45s by an open, visible tab (PresenceHeartbeat.jsx) — lets
    /auth/users report whether a user is actually active in the app right
    now, not just whether their WhatsApp instance is connected."""
    from app.auth import heartbeat
    if x_user_token:
        heartbeat(x_user_token)
    return {"ok": True}
...
def api_list_users(x_user_token: Optional[str] = Header(None)):
    ...
    # "Connected" here means "has the app open and active right now" (stamped
    # by PresenceHeartbeat.jsx pinging /auth/heartbeat every ~45s while the
    # tab is visible) — NOT whether the user's WhatsApp instance is connected,
    # which is a completely different, unrelated question this column used to
    # answer by mistake (real user confusion, 2026-09-21: someone's WhatsApp
    # session being down made them look "disconnected" here even though they
    # were actively using the app). A stale heartbeat (tab closed, laptop
    # asleep, no ping in over 2x the interval) reads as inactive.
    from datetime import datetime, timedelta
    _ACTIVE_WINDOW = timedelta(seconds=100)
    now = datetime.now()
    for u in users:
        last_active = u.pop("last_active_at", None)
        u["is_connected"] = bool(last_active and (now - last_active) < _ACTIVE_WINDOW)
...
def api_send_message(req: SendMessageRequest, x_user_token: Optional[str] = Header(None)):
    try:
        ...
        # If caller provides an explicit instance (e.g. conversation reply — always
        # the instance that received the last inbound message, see conversations.jsx),
        # use it directly. Deliberately NOT gated on the caller owning that instance:
        # any authenticated user can continue any conversation, since every message
        # is independently stamped with instance_name + sent_by_username regardless
        # of who sends it — that per-message record is what preserves accountability,
        # not blocking the send itself (real ask, 2026-09-21: an admin covering a
        # colleague's conversation was blocked from replying in it at all). This is
        # unrelated to the earlier fix in scheduler.py's automated/scheduled-campaign
...
def api_sync_conversation(company_id: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if _provider == "wasender":
            ...
            from app.providers.legacy.waha import WAHAClient, pick_connected_instance as _waha_pick
    ...
    try:
        ...
        if event in ("message", "message.any"):
            ...
            if from_me:
                ...
                try:
                    from app.providers.legacy.waha import WAHAClient as _WAHAClient
        ...
        if event == _WE.PERSONAL_MESSAGE_RECEIVED.value:
            ...
            if "@lid" in remote_jid and session_api_key:
                try:
                    ...
                    from app.providers.legacy.wasender import WasenderClient as _WSC
            ...
            if session_api_key and message_id and remote_jid:
                try:
                    from app.config import WASENDER_BASE_URL
                    from app.providers.legacy.wasender import WasenderClient as _WSClient
