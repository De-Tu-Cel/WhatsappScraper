...
def heartbeat(token: str):
    """Stamps last_active_at for the calling user — pinged periodically from an
    open, visible tab (see PresenceHeartbeat.jsx) so /auth/users can report
    whether someone is actually using the app right now, not just whether
    their assigned WhatsApp instance happens to be connected (that was the
    previous, unrelated meaning of the "Connected" column — real user
    confusion, 2026-09-21)."""
    _db().users.update_one({"session_token": token}, {"$set": {"last_active_at": datetime.now()}})
