...
def send_welcome_email(to_email: str, display_name: str, username: str, pin: str) -> bool:
    """Sent once, right after an admin creates a new user from the Admin panel,
    so the account is usable immediately without the admin having to relay the
    PIN by hand. The PIN only exists in plaintext at this exact moment — it's
    hashed before it's ever stored, so this is the one chance to include it."""
    html = f"""
<!DOCTYPE html>
<html>
...
<body style="margin:0;padding:24px 16px;background:#0a0f1a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  ...
  <div style="max-width:460px;margin:0 auto;background:#060f0c;border-radius:20px;overflow:hidden;border:1px solid rgba(21,87,245,0.25);box-shadow:0 24px 60px rgba(0,0,0,0.6)">
    ...
    <div style="padding:32px">
      <p style="margin:0 0 6px;font-size:13px;font-weight:700;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:0.08em">Tu cuenta ya está lista</p>
      <p style="margin:0 0 20px;font-size:20px;font-weight:700;color:#fff">Bienvenido, {display_name} 👋</p>
      <p style="margin:0 0 24px;font-size:14px;color:rgba(255,255,255,0.6);line-height:1.6">
        Un administrador acaba de crear tu cuenta. Aquí están tus datos de acceso:
      </p>

      <div style="background:rgba(21,87,245,0.1);border:1.5px solid rgba(21,87,245,0.4);border-radius:14px;padding:20px;margin-bottom:20px">
        <div style="font-size:11px;color:rgba(255,255,255,0.3);font-weight:700;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:6px">Usuario</div>
        <div style="font-family:'Courier New',Courier,monospace;font-size:20px;font-weight:800;color:#fff;margin-bottom:16px">{username}</div>
        <div style="font-size:11px;color:rgba(255,255,255,0.3);font-weight:700;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:6px">PIN</div>
        <div style="font-family:'Courier New',Courier,monospace;font-size:28px;font-weight:900;letter-spacing:0.3em;color:#fff;text-shadow:0 0 20px rgba(21,87,245,0.6)">{pin}</div>
      </div>
      ...
      <div style="background:rgba(250,204,21,0.06);border:1px solid rgba(250,204,21,0.2);border-radius:10px;padding:12px 16px;margin-bottom:16px">
        <p style="margin:0;font-size:13px;color:rgba(255,255,255,0.5);line-height:1.5">
          🔒&nbsp; Guarda estos datos en un lugar seguro. Puedes cambiar tu PIN cuando quieras desde tu cuenta.
        </p>
      </div>
    </div>
    ...
    """
    return _smtp_send(to_email, "Bienvenido · Mystery Shopper", html)
