...
function InteractiveMessage({ interactive, isOut, onReply }) {
  ...
  return (
    <Box>
      ...
      {/* Options render inline, always — real WhatsApp list messages hide theirs
          behind a "View options" tap that opens a separate sheet, but for this
          internal viewer showing them straight in the bubble (like a plain
          buttons message) is more useful for actually working the chat, so
          list/buttons/any-count all use the same inline stacked layout now. */}
      {(type === 'buttons' || type === 'list') && options.length > 0 && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.6, borderTop: `1px solid ${borderColor}`, pt: 0.9 }}>
          {options.map((opt, i) => {
            const isSent = sent === opt
            return (
              <Box key={i} onClick={() => handleSelect(opt)} sx={{
                display: 'flex', alignItems: 'center', gap: 1,
                py: 0.7, px: 1, borderRadius: 2,
                cursor: isOut || sent ? 'default' : 'pointer',
                border: `1px solid ${isSent ? 'rgba(74,222,128,0.5)' : borderColor}`,
                bgcolor: isSent ? 'rgba(74,222,128,0.12)' : 'rgba(255,255,255,0.045)',
                transition: 'background-color 0.15s, border-color 0.15s, transform 0.1s',
                '&:hover': !isOut && !sent ? { bgcolor: 'rgba(255,255,255,0.1)', borderColor: accent, transform: 'translateX(2px)' } : {},
              }}>
                <Box sx={{
                  width: 20, height: 20, borderRadius: '50%', flexShrink: 0,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  bgcolor: isSent ? 'rgba(74,222,128,0.22)' : 'rgba(255,255,255,0.08)',
                  border: `1px solid ${isSent ? 'rgba(74,222,128,0.5)' : 'rgba(255,255,255,0.15)'}`,
                }}>
                  <Typography sx={{ fontSize: '0.62rem', fontWeight: 700, color: isSent ? '#4ade80' : 'rgba(255,255,255,0.5)', lineHeight: 1 }}>
                    {isSent ? '✓' : i + 1}
                  </Typography>
                </Box>
                <Typography sx={{ color: isSent ? '#4ade80' : accent, fontSize: '0.8rem', fontWeight: 600, textAlign: 'left', flex: 1 }}>
                  {opt}
                </Typography>
              </Box>
            )
    ...
    <Box sx={{ display: 'flex', justifyContent: isOut ? 'flex-end' : 'flex-start', mb: 0.8, px: 2 }}>
      <Box sx={{
        ...
        {interactive ? (
          ...
          (() => {
            const doc = documentType(msg.media_content_type, lang === 'en' ? 'Document' : 'Documento')
            const accent = doc.color || (isOut ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.55)')
            return (
          <>
            <Box component="a" href={msg.media_url} target="_blank" rel="noopener"
              sx={{ display: 'flex', alignItems: 'center', gap: 1.1, p: 1, borderRadius: '10px', textDecoration: 'none',
                    bgcolor: isOut ? 'rgba(0,0,0,0.12)' : 'rgba(255,255,255,0.05)',
                    border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', transition: 'background-color 0.15s',
              ...
              <Box sx={{
                ...
                <InsertDriveFileIcon sx={{ fontSize: 18, color: accent }} />
              </Box>
              <Typography sx={{ color: isOut ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.75)', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.02em', wordBreak: 'break-word' }}>
                {doc.label}
              </Typography>
            </Box>
            {body && !/^\[.*\]$/.test(body.trim()) && (
