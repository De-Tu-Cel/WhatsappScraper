...
export default function AnimatedFingerprint({ size = 20, color = '#60a5fa', duration = 1800 }) {
  ...
  useEffect(() => {
    ...
    const anim = animate(scanRef.current, {
      translateY: [0, 48],
      duration,
      ease: 'inOutSine',
      loop: true,
  ...
  return (
    <Box component="svg" viewBox="0 0 24 24" width={size} height={size} sx={{ display: 'block', flexShrink: 0 }}>
      <defs>
        ...
        <mask id={maskId}>
          <rect ref={scanRef} x="0" y="-24" width="24" height="24" fill={`url(#${gradId})`} />
        </mask>
      </defs>
      <path d={FINGERPRINT_D} fill={color} opacity={0.28} />
      <path d={FINGERPRINT_D} fill={color} mask={`url(#${maskId})`} />
    </Box>
  )
}
